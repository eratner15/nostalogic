# The Returns Desk. Teaser trailer. Shooting script.

v2. Punch-up pass. Every changed line is listed in `change-notes.md` beside this file, with before, after, and reason. Shots, beats, timecodes, characters, locations, and actions are unchanged from v1. Only dialogue lines and the wording of action lines moved.

Slot 4, Lights Out. Running time 120 seconds. Staff-writer output against `projects/04-writers-room/out/bibles/L1-the-returns-desk.md` section 12 and `projects/03-block-format/out/block-bible.md` section 8.

Format notes.

- Timecodes in brackets mark each beat boundary and the sub-beats inside it. They are running time from the first frame of picture.
- The file contains no bumper. The site wraps it with the 8-second Porch Intro before and the 3-second Click after. The Lights Out sonic ID never plays inside this file.
- No music bed anywhere. Room tone only: fluorescent hum and the cannery, both quiet.
- Every effect (door bell, doorbell chime, ice drop, lamp click, box drop) is mixed at or below the dialogue peak. The mix reference is Desmond's loudest line. Nothing on the effects bus exceeds it.
- No strobe. The only blinking light in the trailer is the chime's small indicator, which glows once per ring, at most one ring every four seconds.
- The town name is not yet in canon. The world beat names the store and the street. No line depends on the town name.
- Nieves, 12, is never in a frame without Marguerite in it or the swinging door with Dolores behind it. Setup C keeps both. Setup A keeps Marguerite. Nieves is never in Setup B alone.

Cast in this trailer: MARGUERITE SALCEDO, 63. INES RUIZ, 12. TEDDY VANCE, 16. AMBROSE QUAYLE, 74. DOLORES RUIZ, 41 (voice, then on screen). DESMOND NUNES, 41, the customer.

---

```
                        THE RETURNS DESK

                        Teaser. 120 seconds.


[00:00]  BEAT 1. COLD OPEN.


INT. HARDISON'S HARDWARE, RETURNS DESK, NIGHT

SETUP B. Straight down from above the gooseneck lamp. A
white plastic WIRELESS DOORBELL CHIME sits on yellowed
laminate. Beside it, its BUTTON, taped to a scrap of
cardboard with painter's tape. A shoebox lid. A receipt
folded into eighths.

No music. No title. Fluorescent hum.

                    MARGUERITE (O.S.)
              (not looking up)
          Returns?

                    DESMOND (O.S.)
          It rings.

                    MARGUERITE (O.S.)
          They do.

                    DESMOND (O.S.)
          When nobody's there.

A beat. The chime's small indicator glows once. The
DOORBELL RINGS. Two soft notes, mixed no louder than
Desmond's last line.

Off screen, a CARDBOARD BOX lands on a floor. Softer
than the ring.

                    TEDDY (O.S.)
          That's fine. That's fine.

                                            CUT TO:


[00:12]  BEAT 2. WORLD.


INT. HARDISON'S HARDWARE, RETURNS DESK, NIGHT

SETUP A, wide. From the customer's side, slightly low.
MARGUERITE, 63, seated frame right, is writing a manila
tag in a slow hand. Behind her, floor to ceiling, THE
SHELF OF RETURNED THINGS: a wind-up kitchen timer, a
rain gauge, a push broom, a bath plug, a rake, each on a
paper tag. DESMOND, 41, frame left, stands in a raincoat
over a pajama top. His hair has made its own decisions.

                    MARGUERITE
          Hardison's. Ambler Street. Open all
          night. Everything comes back
          eventually.

                    DESMOND
          Is that on the sign?

                    MARGUERITE
          It's on the shelf.

[00:19]  SETUP C. Over Marguerite's shoulder. The front
door. The plate-glass window with HARDISON'S HARDWARE in
reversed gold leaf. Outside: one streetlight, the spur.
INES, 12, sits at the far end of the counter with a
green cloth ledger open and a pen up. Frame right, the
SWINGING DOOR. Through its porthole, a shadow moves on
the dock. The dock INTERCOM clicks.

                    DOLORES (V.O.)
              (through the intercom)
          Nieves. Desk?

                    INES
          Desk.

The intercom clicks off. Nieves taps the ledger's four
columns with the pen, one each.

                    INES (CONT'D)
          Item. Claim. Clue. Cause.

                    MARGUERITE
          She has a system.

                    INES
          You have a stamp.

[00:27]  SETUP B. The rubber stamp comes down on the
form. Wet red ink, hand-cut letters: STORY HEARD.

                                            CUT TO:


[00:30]  BEAT 3. CHARACTER.


INT. HARDISON'S HARDWARE, RETURNS DESK, NIGHT

SETUP A. Desmond leans on the counter with both hands.
Marguerite has not moved. Nieves has leaned in at the far
edge of frame.

                    DESMOND
          It rang eleven times. I counted.
          Nobody on the porch. I looked
          through the peephole.

[00:36]  INSERT. PEEPHOLE POV. Fisheye. One porch light.
An empty porch, a doormat, the DOORBELL BUTTON beside
the door frame. Across the street, one upstairs window
is lit. Propped against the inside of that window, in
focus, a poster board in marker:

                HOW FAR CAN MY SIGNAL GO?

A coat-hanger antenna is taped to the sill below it. The
DOORBELL RINGS, two notes, dialogue level. The porch
stays empty. Hold one second.

                    DESMOND (V.O.)
          I looked through the window. I
          looked under the mat.

[00:41]  BACK TO SETUP A.

                    MARGUERITE
          Good. That's thorough.

                    DESMOND
          My wife said I was being ridiculous
          under the mat.

                    MARGUERITE
          Your wife hasn't heard four hundred
          and twelve of these.

                    INES
          Three questions. Where's the porch.
          Where's the street. Who lives across
          it.

                    DESMOND
          A kid. Why?

Nieves writes something down. Desmond watches her do it.

                    DESMOND (CONT'D)
          Why did she write that down?

                    MARGUERITE
          Store policy says receipt required.
          My policy says story required.

                    DESMOND
          I have the receipt.

                    MARGUERITE
          Then you're halfway.

[00:52]  The FRONT DOOR BELL rings, at talking volume.
AMBROSE QUAYLE, 74, comes in, flat cap already in his
hand, holding a FLASHLIGHT upright like a candle. He
takes the spot beside Desmond as if it were reserved.

                    MR. QUAYLE
          I'd like to return this flashlight.
          It's showing me things.

                    MARGUERITE
          What things?

                    MR. QUAYLE
          Mostly the garage. But at night.

He sets a receipt on the counter and smooths it flat.

                    MR. QUAYLE (CONT'D)
          I have the receipt. It's for a rake.
          I feel it's the same spirit.

                    INES
              (without looking up)
          Cause pending.

                                            CUT TO:


[01:02]  BEAT 4. TURN.


INT. HARDISON'S HARDWARE, RETURNS DESK, NIGHT

SETUP C. Nieves, ledger against her chest, walks the staff
side of the counter to the shelf. Marguerite stays in
frame, seated, a pen's length away. Nieves reads the tags
under each item. Every tag has the item on top and a
cause underneath in Marguerite's hand.

                    INES
          Timer. Grandmother wound it. Rain
          gauge. Neighbor's sprinkler. Bath
          plug. Cat. Rake.
              (reads)
          Gate. Wind.

                    MARGUERITE
          Mr. Quayle's.

                    MR. QUAYLE (O.S.)
          I still feel it was the gate.

[01:10]  SETUP B. Nieves's hand stops at the top corner of
the shelf. The oldest tag. Brown at the edges. The
string has gone gray. Marguerite's handwriting, younger,
across the top: nothing. The tag is blank.

                    INES (O.S.)
          This one doesn't say.

                    MARGUERITE (O.S.)
          No.

                    INES (O.S.)
          What was it?

                    MARGUERITE (O.S.)
          Pending.

                    INES (O.S.)
          Since when?

                    MARGUERITE (O.S.)
          Since before the stamp.

[01:19]  SETUP C, wide enough to take in the ICE MACHINE
at frame left. TEDDY, 16, stands beside it, both hands
on a push broom, staring straight at it. Waiting.

The ICE MACHINE drops a load. A bucket of gravel on a
tin roof, mixed under the dialogue.

Teddy jumps anyway. All of him. The broom does not.

                    TEDDY
          I saw it. I saw it coming. It
          doesn't help.

Nobody at the desk turns around. Nieves is still looking
at the blank tag.

                    INES
          Marguerite.

                    MARGUERITE
          Pending.

                                            CUT TO:


[01:28]  BEAT 5. BUTTON.


INT. HARDISON'S HARDWARE, RETURNS DESK, NIGHT

SETUP A. Desmond has the lid back on the shoebox. The
SWINGING DOOR opens. DOLORES, 41, in a canvas work apron,
one glove on, holds the door with her shoulder.

                    DOLORES
          Nieves. Ledger. Coat.

Nieves closes the ledger.

                    INES
          Cause pending.

                    DOLORES
          Coat's not.

[01:35]  The ICE MACHINE drops. Nobody jumps. Not Nieves,
not Marguerite, not Dolores, not Desmond.

                    TEDDY (O.S.)
          See. Fine.

Desmond picks up the shoebox and stops at the counter's
short leg, beside the service bell. He looks at the box.
He looks at the shelf.

                    DESMOND
          So is it haunted or not? I have to
          tell my wife something.

                    MARGUERITE
          Do you have the receipt?

                    DESMOND
          It's haunted.

                    MARGUERITE
          Then I'll need two receipts.

[01:45]  Hold on Desmond. Something in his face lets go.
It is the first time tonight.

Hold on Marguerite. She does not explain. She pulls a
fresh manila tag off the string.

Desmond goes out. The DOOR BELL rings for an exit. The
lamp is warm on the empty counter.

                                            CUT TO:


[01:52]  BEAT 6. TITLE CARD.


SETUP B. Marguerite's hand on the gooseneck lamp switch.
The lamp CLICKS off. A recorded switch, at dialogue
level. The shelf goes dark except the paper tags, which
catch the streetlight through the window.

Over the dark shelf, plain type, no animation:

                     THE RETURNS DESK

                        Lights Out

                Premieres Friday on [BRAND].

               The account holder is a parent.

No music. No voice. No call to action.

[02:00]  END OF FILE.
```

---

## The five tests

1. **Question test.** A viewer writes: "What is the blank tag on the shelf, and why won't she say?" Three viewers write the same sentence because the turn holds on the tag, asks it out loud three times, and the only answer is "Pending."
2. **Two-Laugh test.** The 35-year-old laughs at Mr. Quayle's "I have the receipt. It's for a rake. I feel it's the same spirit," and at the STORY HEARD stamp in close-up. The 6-year-old laughs at Teddy jumping at the ice machine while staring straight at it, then at nobody jumping the second time. Neither laugh needs the other.
3. **Cold test.** A doorbell on a counter, two voices, one ring, one box dropping, "That's fine." No title, no brand, no music. The viewer asks "what is this?" because a doorbell should not be on a counter and should not ring there.
4. **Dialogue test.** Non-dialogue seconds: the stamp close-up (3), the blank tag hold before Nieves speaks (1), the exit hold (7), the title card (8). Nineteen seconds of 120. The peephole insert runs under Desmond's voice. Dialogue is 84 percent of running time.
5. **Standards test.** No child call to action on the card. No copied expression. No effect above dialogue peak. No strobe. Nieves has Marguerite in every frame she is in, and Dolores is behind the porthole or in the doorway. The doorbell does one thing and never a second. The `standards` agent has the final word.

## The fair clue

The cause of the doorbell is a kid across the street with a science project. The clue is on screen at [00:36] in the peephole insert: the poster board that reads HOW FAR CAN MY SIGNAL GO? and the coat-hanger antenna on the sill, in focus, before Nieves asks "Who lives across it." The trailer never names the cause. A parent who pauses can. That is the show's fair-clue rule, kept in the trailer.

## The mild scare and its landing

The scare is a doorbell that rings on an empty porch, seen through a peephole, at talking volume. The object that made the sound is on screen when it sounds. The landing is Dolores through the swinging door, the ice machine dropping with nobody jumping, Teddy's "See. Fine," and a customer who leaves lighter with the lamp warm behind him.

## Standards self-read

Walked against `canon/STANDARDS.md` section 2, items 1 to 15: none present. Section 3: scare resolves with safety and humor (1), Nieves's "Cause pending" to Dolores lands as affection and Dolores answers in kind (2), no body jokes (3), the talk-after moment is the blank tag (4), no screen-time joke (5), Desmond mentions a wife and Nieves has one parent on screen, neither is a joke (6), no food (7), every effect under dialogue peak (8), one indicator glow per ring, no strobe (9). Section 4: no em dashes, the card states the account holder is a parent, no call to action. Passes on my read. Not a substitute for the `standards` agent's pass.
